...
@Dao
interface FlavorProfileDao : BaseDao<FlavorProfile> {
    @Query("SELECT * FROM flavor_profiles WHERE id = :id LIMIT 1")
    suspend fun getFlavorProfileById(id: Long): FlavorProfile?

    @Query("SELECT * FROM flavor_profiles")
    suspend fun getAllFlavorProfiles(): List<FlavorProfile>
}
